...
The builder supports two input modes. Select the mode that matches your CSV structure (via the CLI `--mode` flag or the GUI mode dropdown).

### Vocabulary CSVs

- Provide one Japanese term per row in the first non-empty column.
- Common delimiters (comma, tab, semicolon) are auto-detected.
- Whitespace-only rows are ignored.
...
- Include a header row containing at least `question`, `explanation`, `example_jp`, and `example_en` (case-insensitive; punctuation is ignored).
- Additional columns are ignored.
- If a header is omitted, the builder falls back to assuming the required fields appear in the order listed above.
- Rows without a `question` value are skipped because Anki requires the first field to contain text.

Example `grammar.csv`:

```csv
Question,Explanation,Example_JP,Example_EN
〜ている,Describes an ongoing action,宿題をしている,They are doing their homework.
```
...
1. **Read rows** – Depending on the mode, the builder parses vocabulary terms (`read_csv_single_column`) or grammar entries (`read_grammar_csv`).
2. **Enrich data** (per vocabulary term):
...
| `--mode [vocabulary|grammar]` | Choose between API-enriched vocabulary cards and locally generated grammar explanation cards. | `vocabulary` |
...
1. Prepare your CSV (`terms.csv` for vocabulary or `grammar.csv` for grammar).
2. Run the builder:
   ```bash
   python anki_deck_builder.py build --csv-path terms.csv --output-dir ./anki_out --deck-name "JLPT N5"
   ```
   For grammar sheets, swap in your file path and pass `--mode grammar`.
...
- Switching between vocabulary and grammar requires either separate config files or deleting/regenerating the config so the stored mode matches your target build.
